...
class List extends PureComponent {
  ...
  goDetails = (data, progress) => {
    const { navigation } = this.props
    let { list } = this.state
    list.filter(item => item.isPaused = true)

    navigation.navigate(data.isModal ? 'VModal' : 'ListDls', {
      ...data,
      ...progress
    })
...
class VideoCell extends PureComponent {
  ...
  componentDidUpdate () {
    ...
    if (isPaused) {
      this.VideoModal && this.VideoModal.onSmallStopListPlay()
    }
  }
  ...
  _onProgress = e => this.progress = e

  _onEnd = (screen) => {
    console.log('_onEnd screen:', screen)
    if (screen === 'full') {
      this.VideoModal && this.VideoModal.onCloseModal()
    }
  }

  render () {
    const { navigation, data, idx, goDetails } = this.props
    return (
